...
import keyboard.skin66998213.skins.Home;
 ...
 * out as appropriate.
 */
public class SkinsKeyboard extends InputMethodService implements
    KeyboardView.OnKeyboardActionListener
{
  ...
  public void onCreate() {
    ...
    if (version >= android.os.Build.VERSION_CODES.JELLY_BEAN) {
      Toast toast = Toast.makeText(getBaseContext(), "Keyboard activated!",
          Toast.LENGTH_SHORT);
      toast.setGravity(Gravity.CENTER_VERTICAL, 0, 0);
      toast.show();
    }
    else {
      System.out.println("the background cant be changed");
      Toast toast = Toast.makeText(getBaseContext(),
          "Your device isn't compatable for setting your own background."
              + " You must update to at least android 4.1", Toast.LENGTH_LONG);
      toast.setGravity(Gravity.CENTER_VERTICAL, 0, 0);
      toast.show();
    }
  }
  ...
  public void onInitializeInterface() {
    System.out.println("onInitializeInterface in keyboard");
    ...
    mFile = Home.returnFile();
    if (mFile != "") {
      mPicture = new BitmapDrawable(BitmapFactory.decodeFile(mFile));
      System.out.println(mFile + " in onInitializeInterface");
    }
  ...
  public View onCreateInputView() {
    System.out.println("onCreateInputView in keyboard");
    ...
    mInputView.setHapticFeedbackEnabled(true);
    mInputView.setSoundEffectsEnabled(true);

    System.out.println(mFile);
    int version = android.os.Build.VERSION.SDK_INT;
    if (version >= android.os.Build.VERSION_CODES.JELLY_BEAN) {
      if (mPicture != null) {
        mInputView.setBackground(mPicture);
      }
      else {
        mInputView.setBackgroundColor(000000);
      }
    }
  ...
  private void checkPref() {
    ...
    sound = myPref.getBoolean(getString(R.string.sound_name), true);
    vibrate = myPref.getBoolean(getString(R.string.vib_name), true);
    //autoCorr = myPref.getBoolean(getString(R.string.auto_name), false);
  ...
  public void onKey(int primaryCode, int[] keyCodes) {
    ...
    if (vibrate) {
      // Makes the device vibrate on key press
      Vibrator vib = (Vibrator) getSystemService(VIBRATOR_SERVICE);
      vib.vibrate(45);
    }
    
    if (sound) {
      // This will play the keyboard sound
